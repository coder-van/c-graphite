...
func ReadConfigSections(file string) ([]*configparser.Section, error) {
...
// Create a new WhisperFile database file and write it's header.
func Create(
	path string,
	retentions Retentions,
	aggregationMethod AggregationMethod,
	xFilesFactor float32) (whisper *WhisperFile, err error) {
...
*/
func Open(path string) (whisper *WhisperFile, err error) {
...
func (whisper *WhisperFile) writeHeader() (err error) {
...
*/
func (whisper *WhisperFile) Close() {
...
*/
func (whisper *WhisperFile) Size() int {
...
*/
func (whisper *WhisperFile) MetadataSize() int {
...
/*
  ...
  fail immediately.
*/
func (whisper *WhisperFile) Update(value float64, timestamp int) (err error) {
	diff := int(time.Now().Unix()) - timestamp
	if !(diff < whisper.maxRetention && diff >= 0) {
		return fmt.Errorf("%s", "Timestamp not covered by any archives in this database")
	}
...
func (whisper *WhisperFile) UpdateMany(points []*TimeSeriesPoint) {
...
func (whisper *WhisperFile) archiveUpdateMany(archive *archiveInfo, points []*TimeSeriesPoint) {
...
*/
func (whisper *WhisperFile) getPointOffset(start int, archive *archiveInfo) int64 {
	baseInterval := whisper.getBaseInterval(archive)
	if baseInterval == 0 {
...
func (whisper *WhisperFile) getBaseInterval(archive *archiveInfo) int {
...
func (whisper *WhisperFile) lowerArchives(archive *archiveInfo) (lowerArchives []archiveInfo) {
...
func (whisper *WhisperFile) propagate(timestamp int, higher, lower *archiveInfo) (bool, error) {
...
func (whisper *WhisperFile) readSeries(start, end int64, archive *archiveInfo) []dataPoint {
	var b []byte
	if start < end {
...
*/
func (whisper *WhisperFile) StartTime() int {
	now := int(time.Now().Unix()) // TODO: danger of 2030 something overflow
...
*/
func (whisper *WhisperFile) Fetch(fromTime, untilTime int) (timeSeries *TimeSeries, err error) {
	now := int(time.Now().Unix()) // TODO: danger of 2030 something overflow
...
// read offset of archive
func (whisper *WhisperFile) readInt(offset int64) (int, error) {
...
*/
func ParseRetentionMap(retentionDef string) (*Retention, error) {
...
func ParseRetentionMaps(retentionDefs string) (Retentions, error) {
	retentions := make(Retentions, 0)
	for _, retentionDef := range strings.Split(retentionDefs, ",") {
		retention, err := ParseRetentionMap(retentionDef)
		if err != nil {
			return nil, err
		}
...
/*
  baseInterval is interval when archive-offset write file
  interval is archive.secondsPerPoint*N,N is int
*/
...
/*
	...
	mod(11, 4) = 11 - 4 * floor(11/4) = 11 - 4 * 2 = 3
*/
