...
type Conn struct {
	net.Conn // The inner net.Conn

	PrefixLength byte // The frame size: 1, 2 or 4.

	Endianess binary.ByteOrder // The endianess of the length prefix.

	MaxFrameSize uint // The maximum size of a frame for the current prefix size.

	bytesLeft int // The remaining bytes of the current frame
}
